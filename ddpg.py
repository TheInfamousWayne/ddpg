...
class DDPG(object):
    def __init__(self, env, args): #(self, nb_states, nb_actions, args):
        ...
        self.random_process = OrnsteinUhlenbeckProcess(size=self.nb_actions, theta=args.ou_theta, mu=args.ou_mu, sigma=args.ou_sigma)
    ...
    def random_action(self):
        action = self.env.action_space.sample() #np.random.uniform(-1.,1.,self.nb_actions)
        self.a_t = action
        return action
    ...
    def load_weights(self, output):
        ...
        if not os.path.exists(str(output)+"/actor.pkl"): return
